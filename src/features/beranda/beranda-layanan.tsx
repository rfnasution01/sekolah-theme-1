          ...
          {isLoaded ? (
            layanan?.map((item, idx) => (
              <Link
                to={item?.url}
                target="_blank"
                className={`flex h-[40vh] w-[15%] flex-shrink-0 flex-col items-center justify-center gap-16 rounded-2xl border ${bgPrimary100(color)} p-32 text-center shadow hover:shadow-xl phones:w-3/5`}
                key={idx}
              >
