...
export function CardType6({ agenda }: { agenda: BerandaType }) {
            ...
            agenda?.berita?.slice(0, 3).map((item, idx) => (
              ...
              >
                <div
                  ...
                  className={`h-full w-1/6 rounded-2xl bg-orange-400 p-24 text-center text-[2.4rem] text-white`}
                >
                  {dayjs(item?.tanggal).locale('id').format('DD MMM YYYY')}
                </div>
                <div
                  className="h-full w-5/6 text-[2.6rem] font-bold"
                  style={{ lineHeight: '130%' }}
                >
                  {item?.judul}
                </div>
